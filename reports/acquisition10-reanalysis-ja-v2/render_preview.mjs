...
const html = `<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>仕様書内の不整合とAI実装の消費トークン・評価結果</title><style>${css}</style></head><body><main>${marked.parse(markdown, {gfm: true})}</main></body></html>`;
const preview = path.join(base, 'report-preview.html');
