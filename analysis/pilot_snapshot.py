...
def render(source, output):
    ...
    usage.set_title('使用量：両Runで原本と一致', loc='left', fontsize=13, pad=20)
    ...
    quality.text(.03, .30, '公開仕様が許す自己登録経路を\n採点器が扱えなかったため、採点を無効化。\n\n品質の値・散布図の点は確定できない。', fontsize=11, linespacing=1.6, color='#4B5563', transform=quality.transAxes)
    fig.suptitle('パイロット各1回で停止：使用量は確定、品質は未確定', x=.03, ha='left', fontsize=17)
    fig.supxlabel('両Runとも agent_completed / usage欠測なし / 固定分母57 / 追加比較0 Run\n出典: pilot-runs.csv・pilot-provenance.json  |  品質未確定は0点を意味しない', fontsize=10)
