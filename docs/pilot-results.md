...
## 2026-09-06 更新: 原本欠落の区切りと限定再開

旧pilot-1の2 Runは「原本欠落で評価不能」として区切る。品質null、使用量は過去照合の記録である。workingを代用せず、新しい実装の品質と結合しない。原因は未確定。詳細は[欠落記録](original-loss-incident.json)。

現在の方針は[原本保全・復元手順](preservation.md)とD009。旧予定表と過去記録は履歴として残す。復元試験の合格を条件に、新しいpilot-2-normal/anti各1件だけを許可し、結果整理後に停止する。以前の「原本回復を待ち続ける」手順と「全新規Run禁止」はこの範囲で更新する。試験合格・実施完了は文書追加だけでは意味せず、Issue #1/#11の実測証拠で確認する。
