...
校正概要schema 2はUI挙動の校正版と起動分類の校正版を個別に記録する。旧版の実測を新版の実行結果へ付け替えない。各記録に実行時の評価器ハッシュを保持する。

画面操作では可視ラベルと入力欄の対応、標準のアクセシブル名、同じ申請・ユーザーの表示領域を使う。ラベルのfor/id結合、説明文を含まないラベル、装飾のない操作名、表形式、タイトル自体のリンク、数字だけのページ送り、件数の語順を追加要件にしない。複数入力のどれか判別できない場合は推測して入力しない。非同期に現れるフォーム・一覧・確認画面も待機する。実提出の採点中に判明したUI前提の誤りは原本を保全して無効記録を残し、提出物を変更せず両条件を同じ是正版で再評価する。

今回のpilotでは、校正完了後の実提出評価で追加の登録経路の前提違反が見つかった。公開F-001は自己登録も許容するが、評価器は管理者のユーザー管理画面からの登録経路だけを扱っていた。実提出の品質は未確定（null）として停止し、校正fixtureの成功を品質点の根拠に代用しない。今後再開する場合の最小是正は、公開入口から自己登録経路も解決できるようにして独立校正し、新しい版で同じ固定提出物の両条件を再評価すること。今回の範囲では行わない。
