...
def analysis_sources(root, validity):
    ...
    for name in ('dispatches','plan-revisions','parallel-control.json',
                 ...
                 'reanalysis-source.json','reanalysis-history','pre-v3-experiment.json',
    ...
    for slot in index['runs']:
        ...
        for name in ('assignment.json','worker-started.json','execution-result.json','execution-config.json','acquisition-completion.json','evaluation-jobs','recovery-history'):
