...
def main(assignment_file,secret):
    ...
    if config.get('phase') == 'data-acquisition':
        if config.get('acquisition_policy') == 'parallel-preserve-first-v1':
            from parallel_acquisition import assigned_config
            c = assigned_config(root, config, row)
            if read(directory/'execution-config.json') != c:
                raise ValueError('Pinned execution settings changed')
        else:
